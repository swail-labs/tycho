...
_SLASH_SUBCOMMANDS = {
    ...
    "rewrite": ("Is the runner rewrite on? (a piped runner's real exit status gets recorded)", "rewrite"),
    "rewrite-on": ("Turn ON the runner rewrite — a piped runner goes through `tycho exec` so its real exit status is recorded", "rewrite --on"),
    "rewrite-off": ("Turn OFF the runner rewrite — commands run exactly as written (the default)", "rewrite --off"),
